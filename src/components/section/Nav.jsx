...
const Nav = ({isClick}) => {
  return (
    <div className="w-full h-full p-1 flex flex-col items-center justify-center fixed right-0 top-0 bg-white transition z-20">
      <div className="w-96 h-[40rem] red flex items-start flex-col justify-center gap-5 px-12">
        {navigation.map((data, idx) => {
          return <Route key={idx} routes={data} />;
        })}
      </div>
      <div className="red w-96 h-40 flex items-center justify-center flex-col">
        <p className="text-base">Social Platform</p>
        <div className="w-full h-full red flex items-center justify-evenly">
          <h1>1</h1>
          <h1>2</h1>
          <h1>3</h1>
          <h1>4</h1>
